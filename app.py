...
min_ev = st.sidebar.slider("Minimum Expected Value (+EV %)", min_value=0.0, max_value=15.0, value=1.0, step=0.5,
                           help="Lower this threshold if you want to find more potential betting slips.")
...
if uploaded_file is not None:
    ...
    if 'MatchDate' in matches_df.columns:
        ...
        if len(unique_dates) >= days_ahead:
            ...
            fixtures_df = matches_df.tail(60).copy()
    else:
        fixtures_df = matches_df.tail(60).copy()

if st.button(f"Generate {days_ahead}-Day Value Betting Slips 🔥"):
    st.write(f"Scanning fixtures for positive Expected Value (+EV >= {min_ev}%) across {days_ahead} consecutive days...")
    ...
    if not slips_df.empty:
        ...
        st.warning(f"No bets found meeting the +EV >= {min_ev}% threshold in this batch. Try lowering the Minimum Expected Value slider in the sidebar!")
